...
test.describe('SRS-250 - SDS-250', () => {
  ...
  const handleCopyPopups = async () => {
    const conflictPopup = copyPage.page.getByRole('dialog').filter({ hasText: 'Existing Annotations Found' }).first();
    const unsavedPopup = copyPage.page.getByRole('dialog').filter({ hasText: 'Unsaved Changes' }).first();
    let popupHandled = false;
    if (await conflictPopup.isVisible({ timeout: 3000 }).catch(() => false)) {
      await copyPage.replaceAnnotationsInConflictModal();
      popupHandled = true;
    } else if (await unsavedPopup.isVisible({ timeout: 3000 }).catch(() => false)) {
      await copyPage.clickUnsavedChangesDiscardButton();
      popupHandled = true;
    }
    return popupHandled;
  };
  ...
  test('UTC-2620: Verify keyboard shortcut is supported for copying annotations when the user is viewing an image with saved annotations', async () => {
    ...
    const popupHandled = await handleCopyPopups();
  ...
  test('UTC-2623: Verify annotations are copied to selected image via manual option when the image selection popup is displayed', async () => {
    const sourceIndex = await copyPage.findAnnotatedImageWithNext();
    const sourceCount = await copyPage.getAnnotationCount();
