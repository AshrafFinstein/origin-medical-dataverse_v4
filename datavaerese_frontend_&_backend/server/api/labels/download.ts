...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (!result || !result.excelFile) {
      throw createError({
        statusCode: 404,
        message: 'Labels data not found or Excel file generation failed'
      })
    }

    // Convert base64 back to buffer
    const excelBuffer = Buffer.from(result.excelFile, 'base64')
    const fileName = result.fileName || 'labels_data.xlsx'
    ...
    throw createError({
      statusCode: 500,
      message: e.message || 'Failed to download labels data'
    })
  }
})
